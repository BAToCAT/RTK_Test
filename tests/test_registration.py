...
from settings import reg_first_name, reg_last_name, reg_mail, reg_pwd, reg_pwd_rep, reg_region, test_pwd, invalid_mail, test_pwd_chars_oly, different_pwd, Chinese_char, Latin, Spec_char
...
def test_allready_registred(driver):
    ...
    allready_allert = driver.find_element(By.XPATH, "//*[text()='Учётная запись уже существует']").text
    assert "Учётная запись уже существует" in allready_allert
    WDW(driver, timeout=3).until(EC.presence_of_element_located((By.CSS_SELECTOR, "*[name='firstName']")))

def test_epty_name(driver):
    page = RegPage(driver)
...
def test_password_lover_case(driver):
    page = RegPage(driver)
...
def test_different_paswords(driver):
    page = RegPage(driver)
